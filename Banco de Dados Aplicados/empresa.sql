...
/*Selecione os funcionários em que o cpf do supervisor não é nulo ou ganham mais que 35000*/
select * from funcionario where cpf_supervisor is not null >35000;

/*Selecione os funcionários homens em que a data de nascimento é maior que 01/01/1962.*/
select * from funcionario where sexo = 'M' and datanasc > '1962-01-01';

/*Selecione os funcionários que têm o sobrenome ‘Silva’ ou que o nome seja ‘Jorge’.*/
select * from funcionario where unome ='Silva' or 'Jorge';

/*Selecione os funcionários que não pertencem a nenhum departamento.*/
select * from funcionaro where departamento is null;

/*Selecione os departamentos que possuem gerente.*/
select * from departamento where cpf_gerente;

/*Selecione o departamento de nome Matriz;*/
select * from departamento where dnome ='Matriz';

/*Selecione o departamento que não possui data inicio gerente cadastrada*/
select * from departamento where data_inicio_gerente is null;

/*Selecione as dependentes que são esposas;*/
select * from dependente where parentesco ='Esposa';

/*Selecione as dependentes que são esposas e filhas;*/
select * from dependente where parentesco = 'Esposa' and 'Filha';

/*Selecione os dependentes em que o cpf do funcionário é 12345678966;*/
select * from dependente where fcpf = 12345678966;

/*Selecione os dependentes que são homens ou filhos.*/
select * from dependente where sexo = 'M' or 'Filho';

/*Selecione os projetos localizados em São Paulo;*/
select * from projeto where projlocal ='São Paulo';

/*Selecione os projetos localizados em São Paulo e que o departamento seja 5 ou os projetos localizados em São Paulo e que o departamento seja 1.*/
select * from projeto where projlocal ='São Paulo' and projnumero = 5 or projlocal ='São Paulo' and projnumero = 1;
