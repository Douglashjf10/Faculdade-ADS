...
select p.id_produto, p.nome, p.estoque, preco_unitario
from produto p
...
select pv.nr_venda, count(pv.id_produto) as
quantidade_produto_vendidos
from produto_vendido pv
group by pv.nr_venda
having count(pv.id_produto) > 5
order by quantidade_produto_vendidos desc;
